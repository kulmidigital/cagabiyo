...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      <DialogContent
        className={cn(
          'top-[10vh] max-h-[80vh] translate-y-0 overflow-hidden p-0',
          className,
        )}
        showCloseButton={showCloseButton}
      >
        {/* Inside the popup so it can actually label the dialog — outside it,
            the title is never associated with the dialog element. */}
        <DialogHeader className="sr-only">
          <DialogTitle>{title}</DialogTitle>
          <DialogDescription>{description}</DialogDescription>
        </DialogHeader>
        {/* cmdk's Input/List/Item all read the command store from context, so
            they must be wrapped. Without this, `store.subscribe` is undefined
            and the dialog throws the moment it opens. */}
  ...
  return (
    <div data-slot="command-input-wrapper" className="p-1">
      <InputGroup className="border-transparent border-b-input bg-transparent px-3">
        ...
        />
        <InputGroupAddon>
          <FiSearch className="size-3.5 shrink-0 opacity-50" />
        </InputGroupAddon>
      </InputGroup>
    </div>
  )
...
function CommandItem({
  ...
  return (
    ...
    >
      {children}
      <FiCheck className="ml-auto opacity-0 group-has-data-[slot=command-shortcut]/command-item:hidden group-data-[checked=true]/command-item:opacity-100" />
    </CommandPrimitive.Item>
  )
}
