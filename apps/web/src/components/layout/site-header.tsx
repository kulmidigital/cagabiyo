        ...
        {activeSection ? (
          <div
            ...
            className={cn(
              ...
              'mx-auto hidden overflow-hidden border border-t-0 border-transparent bg-ink-950/95 backdrop-blur-2xl lg:block',
              shape,
              scrolled
                ? 'border-b-white/10'
                : 'border-white/15 shadow-[0_24px_60px_-24px_rgb(4_14_29/0.7)]',
            )}
          >
...
function MegaPanelContent({ section }: Readonly<{ section: NavSection }>) {
  return (
    <div className="grid gap-10 py-6 sm:py-10 lg:grid-cols-12 lg:gap-14">
      <div
        className={cn(
          'grid gap-10 sm:grid-cols-2',
          section.feature ? 'lg:col-span-8' : 'lg:col-span-12 lg:grid-cols-4',
        )}
      >
        {section.columns?.map((column) => (
          <div key={column.heading}>
            <p className="eyebrow mb-5 text-signal-400">{column.heading}</p>
            <ul className="space-y-0.5">
              {column.links.map((link) => (
                <li key={link.to}>
                  <SmartLink
                    href={link.to}
                    className="group -mx-3 block px-3 py-2.5 transition-colors hover:bg-white/5"
                  >
                    <span className="flex items-center gap-2 text-sm font-semibold text-white">
                      {link.label}
                      <FiArrowUpRight
                        className="size-3.5 -translate-x-1 text-signal-500 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100"
                        aria-hidden="true"
                      />
                    </span>
                    {link.description ? (
                      <span className="mt-0.5 block text-[0.8125rem] leading-snug text-ink-300">
                        {link.description}
                      </span>
                    ) : null}
                  </SmartLink>
      ...
      {section.feature ? (
        <div className="lg:col-span-4">
          <div className="h-full border border-white/12 bg-linear-to-br from-ink-600/50 to-ink-900/50 p-7">
